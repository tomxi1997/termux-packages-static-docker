...
termux_step_make() {
	...
	#make -j ${TERMUX_PKG_MAKE_PROCESSES} man)
        CGO_ENABLED=0 SHIM_CGO_ENABLED=0 make -j ${TERMUX_PKG_MAKE_PROCESSES}
        #CGO_ENABLED=0 SHIM_CGO_ENABLED=0 make -j ${TERMUX_PKG_MAKE_PROCESSES}
	(unset GOOS GOARCH CC CXX CFLAGS CXXFLAGS LDFLAGS
	CGO_ENABLED=0 SHIM_CGO_ENABLED=0 make -j ${TERMUX_PKG_MAKE_PROCESSES} man)
